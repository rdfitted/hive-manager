...
impl InjectionManager {
    ...
    #[allow(dead_code)]
    pub fn notify_evaluator_milestone_ready(
        &self,
        session_id: &str,
        queen_id: &str,
        evaluator_id: &str,
        ...
        if !queen_id.ends_with("-queen") {
            return Err(InjectionError::NotAuthorized(
                "Only Queen can notify the Evaluator".to_string(),
            ));
        }
        if !evaluator_id.ends_with("-evaluator") {
            return Err(InjectionError::NotAuthorized(
                "Milestone notifications must target the Evaluator".to_string(),
            ));
        }

        let coord_message = CoordinationMessage::milestone_ready(
            &format_agent_display(queen_id),
            &format_agent_display(evaluator_id),
            message,
        );
        ...
        self.write_session_peer_message(session_id, |state| {
            state.write_milestone_ready(queen_id, evaluator_id, message)
        })?;

        self.write_to_agent(evaluator_id, message)?;
    ...
    #[allow(dead_code)]
    pub fn notify_queen_qa_verdict(
        &self,
        session_id: &str,
        evaluator_id: &str,
        queen_id: &str,
        ...
        if !evaluator_id.ends_with("-evaluator") {
            return Err(InjectionError::NotAuthorized(
                "Only the Evaluator can report QA verdicts".to_string(),
            ));
        }
        if !queen_id.ends_with("-queen") {
            return Err(InjectionError::NotAuthorized(
                "QA verdicts must target the Queen".to_string(),
            ));
        }

        let coord_message = CoordinationMessage::qa_verdict(
            &format_agent_display(evaluator_id),
            &format_agent_display(queen_id),
            message,
        );
        ...
        self.write_session_peer_message(session_id, |state| {
            state.write_qa_verdict(evaluator_id, queen_id, message)
        })?;

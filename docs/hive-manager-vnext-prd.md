...
### Models and labels
Use canonical model IDs in configuration and launch payloads. The current canonical IDs are `gpt-6-sol` and `fable`; **GPT-6 Sol** and **Fable 5** are display names. A direct Hive recommends an Opus Queen plus one generic Codex `gpt-6-sol` principal; specialized built-in templates can provide backend/frontend principals. The Codex aliases `gpt-6` and legacy `gpt-5.6` are normalized at launch to their Sol model IDs for compatibility. Older models remain selectable, and an explicit operator selection always overrides a recommendation.

...
- **codex**: Performs extensive codebase indexing before producing output (8-12min with no git diff is normal). Performs well once indexing completes. Occasional interactive approval stalls but rarer than indexing delays.
- **gemini**: Also indexes codebase before starting (~7-12min). Good for frontend, data model changes. Reliable once indexing completes.
...
- **Strategy**: Wait at least **12-15 minutes** before declaring a worker stalled. Codex and Gemini index extensively — no git diff for 8-12min is normal startup behavior. Only Droid and Claude start producing output quickly. Check terminal/PTY activity if possible, not just git diff. Only flag as truly stalled if zero terminal activity AND >15min elapsed.
